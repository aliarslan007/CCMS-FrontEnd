...
import UserTableRow from '../user-table-row-client_restore';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'representative_name', label: 'Inactive By', width: 180 },
  { id: 'record_name', label: 'First Name', width: 180 },
  { id: 'company_name', label: 'Company Name', width: 180 },
  { id: 'identification', label: 'Title', width: 180 },
...
export default function UserClinetListView({ moduleName }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (!logSentRef.current) {
        ...
        logActivity('User view Client Records', dynamicModuleName);
  ...
  return (
    <>
      <Container maxWidth="">
        <CustomBreadcrumbs
          heading="Inactive Client Contacts"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'inactive client contacts' },
          ]}
          action={
